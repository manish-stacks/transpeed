import React from 'react'

export default function CoreAdvantage() {
    return (
        <>
            <section className='mt-10 mb-20'>
                <div className="w-full md:px-44 px-10 max-[768px]:px-5 space-y-5">
                    <div class="grid grid-cols-1 gap-4 text-center">
                        <div>
                          <h4 className='text-yellow-500 p-3 text-xl'>Core Advantage</h4>
                          <h2 className='max-w-7xl pl-4 mx-auto text-xl md:text-5xl max-[768px]:text-3xl font-bold font-sans text-center mb-10'>Get the Global Logistics Advantage</h2>
                        </div>
                    </div>

                    <div className='grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-2 text-center gap-6'>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/global-network.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>Global Network</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/leadership.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>Expert Team</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/on-time-delivery.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>On Time Stuffing</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/ino-solutions.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>Innovative Solution</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/All-Major-Ports-and-lcd.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>All Major Ports and ICD</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/On-Time-De-stuffing.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>On Time De-stuffing</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/Cost-Effective.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>Cost Effective</h3>
                            </div>
                        </div>
                        <div className='bg-[#d3edff] p-5 rounded-xl'>
                            <img src='/images/Customer-Oriented-Solution.png' className='flex mx-auto'></img>
                            <div>
                                <h3 className='font-bold text-[16px] xl:text-xl'>Customer Oriented Solution</h3>
                            </div>
                        </div>

                    </div>

                </div>
            </section>
        </>
    )
}
